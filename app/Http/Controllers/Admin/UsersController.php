...
class UsersController extends Controller
{
  ...
  public function index()
  {
    ...
    $users = User::join('roles', 'users.role_id', '=', 'roles.id')
      ->select(['users.id', 'users.username', 'users.email', 'users.image', 'roles.display_name', 'users.slug', 'users.role_id', 'users.approved', 'users.subscription_type', 'users.comission_value'])
  ...
  public function update(Request $request, $slug)
  {
    ...
    $validation = [
      ...
      'image'     => 'bail|mimes:png,jpg,jpeg|max:2048',
      'subscription_type'    => 'bail|required',
      'country_id'    => 'bail|required',
      'state_id'      => 'bail|required',
      'city_id'       => 'bail|required'
    ];
    ...
    if ($request->subscription_type == 'Silver') {
      $comission_value = 10.00;
    } else if ($request->subscription_type == 'Gold') {
      $comission_value = 7.00;
    } else if ($request->subscription_type == 'Platinum') {
      $comission_value = 5.00;
    } else if ($request->subscription_type == 'Diamond') {
      $comission_value = 3.00;
    }
    // dd($comission_value);
    ...
    if (checkRole(getUserGrade(1))) {
      ...
      $record->comission_value = $comission_value;
    }
  ...
  public function show($slug)
  {
    ...
    $record = User::join('roles', 'users.role_id', 'roles.id')
      ...
      ->select([
        'users.*', 'roles.display_name', 'countries.title',
